...
export default function AdminProfilePage() {
  const fallbackName = 'Administrateur TT';
  const fallbackEmail = 'admin@tunisietelecom.tn';
  const { name, email, photoUrl, accessSinceMonthYear, photoUploading, uploadPhoto, updateProfile } = useRoleProfile('admin', {
    name: fallbackName,
    email: fallbackEmail,
  });
          ...
          <div style={{
            ...
            <div style={{ position: 'relative', display: 'inline-block', marginBottom: 16 }}>
              <div style={{
                ...
                boxShadow: '0 8px 24px rgba(230,81,0,0.25)', overflow: 'hidden',
              }}>
                {photoUrl ? (
                  <img src={photoUrl} alt="Photo de profil admin" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  initials
                )}
              </div>
            ...
            <div style={{ marginTop: 14, fontSize: 12, color: '#aaa' }}>
              Admin depuis {accessSinceMonthYear || '--'}
            </div>
          </div>
        </div>
